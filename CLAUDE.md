...
8. ✅ **Publicado no repositório da empresa** (2026-07-26) — `NexGoldCompany/NexGestor` (privado), com README de onboarding na raiz. Histórico auditado antes: sem segredos reais. **⚠️ Desde 2026-07-28 (parte 2), `origin` (pessoal) e `empresa` estão DESSINCRONIZADOS**: os 3 commits desta sessão foram enviados só pro `origin`. O usuário tem `push` mas não é `admin` no repo da empresa — alguém com acesso precisa replicar os commits lá.
...
>
> **Sincronizar `origin` → `empresa`** com os 3 commits desta sessão (`1501bc5`, `82f0cb9`, `1682f14`) — bloqueado por permissão de admin, ver item 8.
>
