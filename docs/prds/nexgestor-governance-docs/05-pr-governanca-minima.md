...
**Estado: implementação documental concluída e validada localmente
(`CLAUDE.md` + pacote `nexgestor-governance-docs/`, incluindo o registro da
sessão de 08/09 com a auditoria reconciliada); não commitada, PR no GitHub
não aberto.** Título sugerido: `docs: define bloqueadores e encerramento
proporcional`. Descrição pronta para uso na seção "Descrição do PR" abaixo.
...
## Descrição do PR (pronta para uso ao abrir)

**Título:** `docs: define bloqueadores e encerramento proporcional`

**Base e dependência documental:** este PR parte de `692a0f9` (branch `main`
local; na última consulta desta sessão, `main` estava 1 commit à frente de
`origin/main`, sem fetch remoto nesta rodada — confirmar antes de abrir). A
política de continuidade/checkpoint/rastreabilidade que este PR complementa
já está em `692a0f9`, não faz parte deste PR. `docs/governanca-minima` foi
criada a partir desse mesmo commit, então hoje ela é idêntica a `main` mais
os arquivos deste PR.

**Ordem de integração recomendada:**
1. `git fetch origin` primeiro e conferir o estado real de `origin/main`
   (ancestralidade em relação a `692a0f9`, divergência e eventuais regras de
   proteção da branch) antes de qualquer push. Não presumir que `origin/main`
   continua em `84a8cf0` nem que push direto é permitido.
2. Integrar a dependência documental (`692a0f9`, 18 arquivos, separada desta
   entrega) antes da publicação: se `origin/main` só estiver atrás e um
   avanço fast-forward for permitido, enviar essa base a `origin/main`. Se
   exigir PR por proteção de branch ou divergência, abrir um PR documental
   próprio só para essa base primeiro, sem contornar proteção nem usar force
   push nem merge automático.
3. Só depois publicar `docs/governanca-minima` e abrir o PR de governança
   contra `origin/main` (já com `692a0f9` integrado) — o diff resultante fica
   isolado a `CLAUDE.md`, `docs/README.md`, ao pacote
   `docs/prds/nexgestor-governance-docs/` e a `docs/sessions/2026-09-08.md`.
   Se a integração da base ainda não tiver sido possível, a preparação local
   deste PR pode seguir independente, deixando explícito que a base ainda
   precisa trocar para `main` depois.

**Problema:** ver "Problema e resultado esperado" acima — a auditoria do
pacote (`docs/sessions/2026-09-08.md`, seção "Auditoria — nexgestor-governance-docs")
encontrou critérios (G01–G09) que podiam classificar quase tudo como crítico,
impedir encerramento por qualquer incerteza e duplicar os estados oficiais
de `CLAUDE.md`.

**Resultado:** `CLAUDE.md` ganhou as seções "Achados, bloqueios e
encerramento" e "Mudanças analíticas, contratos e dados", cobrindo G01–G09
sem criar estados novos. Os documentos 01–04 do pacote passam a se declarar
proposta histórica/referência de auditoria, não política; o README do
pacote aponta `CLAUDE.md` como fonte normativa e o relatório real da
auditoria em `docs/sessions/2026-09-08.md`.

**Validação:** revisão do diff completo contra a especificação, `git diff
--check` sem erros, conferência manual dos links relativos novos. Mudança
exclusivamente documental — sem suíte de produto, sem alteração de
código/teste/configuração, sem chamada paga.
